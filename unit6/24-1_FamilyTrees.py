...
def ancestors(genealogy, person):
    if person in genealogy:
        result = []
        for a in genealogy[person]:
            result.append(a)
            for i in (ancestors(genealogy, a)):
                result.append(i)
    else:
        result = []
    return result
...
#Udacity’s answer is slightly neater using less code, only a single 
#for loop and is not having to initialise the result list.  It also 
#avoids the issue of having to iterate through the list of two `parents` 
#by using the `+` operator to concatenate that list to the exsiting `result`. 
#Nice!  
