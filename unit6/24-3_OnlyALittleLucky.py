...
def ordered_search(index, ranks, keyword):
    ...
    for page in pages:                      # Iterate through pages and create list of
        for key, val in ranks.items():      # tuples containing page and rank
            if key == page:
                rank_list.append([key, val])
                
    for res in (sort_rank(rank_list)):      # Sort list of tuples by rank, then place
        result.append(res[0])               # each page in the result list in
...
def sort_rank(rank_list):
    ...
    if len(rank_list) > 0:
        ...
        for i in rank_list:
            ...
            if i[1] > pivot:
                greater.append(i)
        return sort_rank(greater) + equal + sort_rank(less)  # Sort in reverse order
    return rank_list                                         # with highest rank first
...
def quicksort_pages(pages, ranks):
    if not pages or len(pages) <=1:     # returns our base-case for the recursion
        return pages
    else:
        pivot = ranks[pages[0]] # find pivot (choosing first item in list)
        worse = []
        better = []
        for page in pages[1:]:  # Here the first page (used as the pivot) is not included
            if ranks[page] <= pivot:
                worse.append(page)
            else:
                better.append(page)
